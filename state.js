...
let state={
  ...
  goalTypeFilter:'all',
  projects:load('budget_projects',[]),
  projectFilter:'all',
  projectSelected:null,
  projectExpanded:null,
  homeCardTab:'reminders',
  gratitude:load('budget_gratitude',[]),
  gratMonth:{y:_now.getFullYear(),m:_now.getMonth()},
  habits:load('budget_habits',[]),
  habitFilter:'all',
  habitView:'active',
  habitSelected:null,
  habitMonthYM:{y:_now.getFullYear(),m:_now.getMonth()},
  cycles:load('budget_cycles',[]),
  cycleLogs:load('budget_cycle_logs',[]),
  cycleSettings:load('budget_cycle_settings',{avgCycle:28,avgPeriod:5,luteal:14,showPredict:true}),
  cycleMonth:{y:_now.getFullYear(),m:_now.getMonth()},
...
const THEMES={
  ...
  white:{name:'純白極簡',swatches:['#45423C','#6B6862','#B8B5AE'],
    vars:{'--p':'#45423C','--p2':'#45423C','--p3':'#45423C','--latte':'#45423C','--latte2':'#45423C',
          '--bg':'#FDFCFA','--surface':'#FDFCFA','--text':'#242220','--text2':'#6B6862','--border':'#45423C',
          '--shadow':'rgba(0,0,0,0)','--hdr-grad':'#FDFCFA'}},
  dark:{name:'暗黑極簡',swatches:['#909090','#707070','#505050'],
    vars:{'--p':'#909090','--p2':'#909090','--p3':'#909090','--latte':'#909090','--latte2':'#909090',
          '--bg':'#121212','--surface':'#121212','--text':'#E0E0E0','--text2':'#999999','--border':'#909090',
          '--shadow':'rgba(255,255,255,0)','--hdr-grad':'#121212'}},
};
const FONTS={
